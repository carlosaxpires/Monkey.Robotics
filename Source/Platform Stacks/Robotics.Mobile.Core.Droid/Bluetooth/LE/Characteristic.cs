namespace Robotics.Mobile.Core.Bluetooth.LE
{
    ...
    public class Characteristic : ICharacteristic
    {
        ...
        public Task<GattCommunicationStatus> Write(byte[] data, WriteType writeType)
        {
            ...
            if (writeType == WriteType.WriteWithResponse)
            {
                EventHandler<CharacteristicWriteEventArgs> written = null;
                written = (object sender, CharacteristicWriteEventArgs e) =>
                {
                    // it may be other characteristics, so we need to test
                    tcs.SetResult(e.Status);
                    if (this._gattCallback != null)
                    {
                        // wire up the characteristic value writing on the gattcallback
                        this._gattCallback.CharacteristicValueWritten -= written;
                    }
                };
            ...
            if (writeType == WriteType.WriteWithoutResponse)
            {
                tcs.SetResult(GattCommunicationStatus.Success);
            }
